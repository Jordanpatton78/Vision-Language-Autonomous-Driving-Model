...
1.  Establish baseline performance by adding a regression head to the vision encoder only.
2.  Train the full VLM to produce accurate chain-of-thought reasoning and evaluate its performance on discrete prediction targets identified during EDA.
3.  Add a regression head to the final hidden layer of the VLM to predict continuous vehicle speed and steering angle.
4.  Train a regression head that uses both final token embeddings and image information.
5.  Distill the knowledge from the reasoning model into a smaller, vision-only model that learns by matching embeddings, with the goal of significantly reducing inference latency.
...
| Experiment | Speed Metric | Steering Metric | Latency (s) |
| :--- | :--- | :--- | :--- |
| **9.1 Baseline Vision Encoder** | RMSE: 11.758 mph | RMSE: 3.473° | - |
| **9.2 CoT Discrete Performance** | Acc: 0.5 / F1: 0.48 / MCC: 0.379 | Acc: 0.737 / F1: 0.719 / MCC: 0.130 | - |
| **9.3 Reasoning Regression** | RMSE: 8.075 mph | RMSE: 8.722° | - |
| **9.4 Reasoning (Text + Image)** | RMSE: 3.264 mph | RMSE: 6.015° | 0.127 |
| **9.5 Distilled Regression** | RMSE: 8.600 mph | RMSE: 6.310° | 0.050 |
