...
distilled_head_path = "../models/best_distilled_model.pth"
...
class RegressionHead(nn.Module):
    ...
    def forward(self, return_embeddings=False, **inputs):
        ...
        if return_embeddings:
            return regression_output, regression_inputs
        else:
            return regression_output

class DistilledRegressionHead(nn.Module):
    def __init__(self, image_input_dim=1536, text_input_dim=2048, output_dim=2):
        super().__init__()
        self.output_dim = output_dim
        self.image_input_dim = image_input_dim
        self.text_input_dim = text_input_dim
        self.hidden_size = self.image_input_dim + self.text_input_dim
        self.first_regression_head = nn.Sequential(
            nn.Linear(self.image_input_dim, self.hidden_size)
        )
        self.second_regression_head = nn.Sequential(
            nn.Linear(self.hidden_size, 256),
            nn.ReLU(),
            nn.Dropout(0.1),
    ...
    def forward(self, x, return_embeddings=False):
        intermediate_outputs = self.first_regression_head(x)
        if return_embeddings:
            return self.second_regression_head(intermediate_outputs), intermediate_outputs
        else:
            return self.second_regression_head(intermediate_outputs)

teacher_model = RegressionHead(lora_model).to(device)
teacher_model = teacher_model.to(dtype=torch.bfloat16)
teacher_model.eval()

student_model = DistilledRegressionHead().to(device)
...
optim = AdamW(student_model.parameters(), lr=1e-3)
epochs = 20
patience = 3
epochs_wo_improve = 0
loss_fn = nn.MSELoss()
...
for epoch in range(epochs):
    ...
    for batch in train_dl:
        ...
        with torch.no_grad():
            teacher_outputs, teacher_embeddings = teacher_model(**inputs, return_embeddings=True)

        teacher_speed_preds = teacher_outputs[:, 0:1]

        teacher_steering_preds = teacher_outputs[:, 1:2]

        student_outputs, student_embeddings = student_model(pooled_image_features, return_embeddings=True)

        student_speed_preds = student_outputs[:, 0:1]

        student_steering_preds = student_outputs[:, 1:2]

        targets = torch.tensor([[target_speed, target_steering]], dtype=torch.float32).to(device)

        pred_loss = loss_fn(student_outputs.float(), targets)

        emb_loss = loss_fn(student_embeddings.float(), teacher_embeddings.float())

        loss = (0.5 * pred_loss) + (0.5 * emb_loss)
    ...
    for batch in val_dl:
        with torch.no_grad():
            ...
            outputs = student_model(pooled_image_features)
    ...
    if avg_mse < best_val_mse:
        ...
        torch.save(student_model.state_dict(), distilled_head_path)
        epochs_wo_improve = 0
    else:
        epochs_wo_improve += 1
...
model = DistilledRegressionHead().to(device)
model.load_state_dict(torch.load(distilled_head_path))
...
for batch in test_dl:
    with torch.no_grad():
        ...
        outputs = model(pooled_image_features)
...
print(f"Average latency per inference: {avg_dur}")

# Average speed RMSE: 8.59986400604248
# Average steering RMSE: 6.309962749481201
# Average latency per inference: 0.049733283448547716